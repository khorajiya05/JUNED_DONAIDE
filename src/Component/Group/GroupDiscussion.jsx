...
const GroupDiscussion = (props) => {
  ...
  const { UserID, UserName, Image } = useSelector((state) => state.auth?.loginData);
  ...
  const AddEventConfirmationDetails = async (eventId, going, mayBe, cantGo) => {
    const res = await GroupService.GetEventdetailByUserIdAndEventId(UserID, eventId);

    console.log("res111111111111111111111111", res.data.status === "SUCCESS", res.data);
    ...
    const data = {
      ...
      cantGo: cantGo,
      mayBe: mayBe,
      eventConfirmId: res.data.data !== null ? res.data.data.eventConfirmId : null,
    };
          ...
          <div className="col-xl-9 col-lg-12 col-md-12 col-sm-12 col-12">
              ...
              <div className="create-post-outer-box">
                <div className="create-post-inner-box">
                  <div className="create-post-text">
                    <img
                      src={Image === null || Image === undefined || Image === "" ? process.env.PUBLIC_URL + "/Images/guest-user.jpg" : IMAGE_BASE_URL + Image}
                      alt=""
                    />
                    <textarea
            ...
            {upComingEvents !== undefined && upComingEvents.length > 0
              ? upComingEvents.map((events, index) => (
                <div className="row mb-3 event-section custom-event-section custom-common-ui" key={index}>
                  <div className="col-md-12">
                    <div className="eve-user-name d-flex align-items-center media-posts-outer-box">
                      ...
                      <div className="heading-invitation">
                        <p className="mb-0"><strong>{events.userName}</strong> invied you</p>
                      </div>
                    </div>
                  </div>
                  <div className="col-md-12">
                    <div className="event-image position-relative">
                      <img
                        src={events.coverImage ? `${IMAGE_BASE_URL + events.coverImage}` : process.env.PUBLIC_URL + " /Images/group_cover_img.jpg "}
                        className=" cover-img"
                        alt=""
                      />
                  ...
                  <div className="even-invite-action custom-event-action">
                    <>
                      <button
                        ...
                        onClick={() => AddEventConfirmationDetails(events.eventID, 1, 0, 0)}
                      >
                        <i className="fa fa-check" aria-hidden="true"></i>
                        <p>{events?.eventConfirmations?.going?.count ? events.eventConfirmations.going.count : 0}
                          <span> Going</span>
                        </p>
                      </button>
                      <button
                        className="btn btn-secondary me-2"
                        onClick={() => AddEventConfirmationDetails(events.eventID, 0, 1, 0)}
                      >
                        <i className="fa fa-question-circle" aria-hidden="true" ></i>
                        <p>{events?.eventConfirmations?.maybe?.count ? events.eventConfirmations.maybe.count : 0}
                          <span> Maybe</span>
                        </p>
                      </button>
                      <button
                        className="btn btn-secondary me-2"
                        onClick={() => AddEventConfirmationDetails(events.eventID, 0, 0, 1)}
                      >
                        <i className="fa fa-times" aria-hidden="true"></i>
                        <p>{events?.eventConfirmations?.cantGo?.count ? events.eventConfirmations.cantGo.count : 0}
                          <span> Can't go</span>
                        </p>
                      </button>
                    </>
            ...
            {groupPostData.length > 0
              ? groupPostData.map((item, index) =>

                item.fileType && item.fileType === "Vedio" ? (
                  <div className="media-posts-outer-box" key={index} >
                        ...
                        <div className="meadia-posts-card-header">
                          <div className="meadia-posts-card-profile">
                            <div className="meadia-posts-card-profile-pic">
                              <img
                                src={item.profilePicture ? `${IMAGE_BASE_URL + item.profilePicture}` : process.env.PUBLIC_URL + "/Images/guest-user.jpg"}
                                alt="profile"
                              />
                            </div>
                          ...
                          <div className="player-wrapper">
                            {item.postImageOrVedio ? (
                              <ReactPlayer
                                ...
                                url={item.postImageOrVedio ? `${IMAGE_BASE_URL + item.postImageOrVedio}` : ""}
                                width="100%"
                                height="100%"
                                controls={true}
                              />
                        ...
                        <div className="meadia-posts-card-header">
                          <div className="meadia-posts-card-profile w-100">
                            ...
                            <div className="meadia-posts-card-profile-name d-flex justify-content-between align-items-center w-100">
                              ...
                              {/* <div>
                                    <a href={item.fileName} download ><i className="fa fa-cloud-download" aria-hidden="true"></i></a>
                                  </div> */}
                            </div>
                          </div>
                        </div>
                            ...
                            {item.fileName && item.fileName.length > 0 && item.fileName.split(".")[1] === "pdf" ? (
                              ...
                              <>
                                ...
                                {/* <DocViewer
                                  ...
                                  documents={[{ uri: `${IMAGE_BASE_URL + item.postImageOrVedio}` }]}
                                /> */}
                              </>
                            )}
                          </div>
