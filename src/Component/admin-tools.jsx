...
const AdminTools = () => {
  ...
  const { UserID, RoleID, IsMaster } = useSelector((state) => state?.auth?.loginData)
  ...
  useEffect(() => {
    if (IsMaster === "True" && Number(RoleID) === Number(1)) {
      navigate("/owner-admin-list");
    } else {
      dispatch(getProfileDataActionThunk(UserID));
    }
  }, [dispatch, UserID, RoleID, IsMaster, navigate]);
